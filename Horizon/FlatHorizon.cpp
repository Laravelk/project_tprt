...
namespace ray_tracing {
    ...
    float FlatHorizon::getDepth(std::array<float, 2> x) const {
        ...
        float z = (depth - (absolute_x[0] * normal[0] + absolute_x[1] * normal[1])) / (normal[2] + 1e-16f);
        return z;
    }

    FlatHorizon::FlatHorizon(float depth, float dip, float azimuth, std::array<float, 2> anchor,
                std::string name) :
            dip(dip),
            azimuth(azimuth),
            depth(depth),
    ...
    // две точки и пересечение отрезка между ними с плоскостью. @return точка пересечения
    std::array<float, 3> FlatHorizon::calcIntersect(const std::array<float, 3> &x0, const std::array<float, 3> &x1) const {
        float d = -depth;

        if ((normal[0] * (x1[0] - x0[0]) + normal[1] * (x1[1] - x0[1]) + normal[2] * (x1[2] - x0[2])) == 0)
            return {{NAN, NAN, NAN}};

        float lambda = -(normal[0] * x0[0] + normal[1] * x0[1] + normal[2] * x0[2] + d)
                       / (normal[0] * (x1[0] - x0[0]) + normal[1] * (x1[1] - x0[1]) + normal[2] * (x1[2] - x0[2]));
        ...
        std::array<float, 3> vec0{x0[0] - intersect[0], x0[1] - intersect[1], x0[2] - intersect[2]};
        std::array<float, 3> vec1{x1[0] - intersect[0], x1[1] - intersect[1], x1[2] - intersect[2]};
    ...
    FlatHorizon FlatHorizon::fromJSON(const rapidjson::Value &doc) {

        if (!doc.IsObject())
            throw std::runtime_error("FlatHorizon::fromJSON() - document should be an object");

        std::vector<std::string> required_fields = {"HType", "Dip", "Azimuth", "Depth", "Anchor", "Cardinal", "Name"};
        ...
        if (!doc["Depth"].IsFloat())
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `Depth` should be a float");

        if (!doc["Anchor"].IsArray())
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `Anchor` should be an array");

        // TODO: убрать комментарий
        //if (doc["Anchor"].Size() != 2)
       //     throw std::runtime_error("FlatHorizon::fromJSON - invalid JSON, wrong 'Anchor' size (should be equal two)");

        if (!doc["Cardinal"].IsString())
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `Cardinal` should be a string");

        if (!doc["Name"].IsString())
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `Name` should be a string");

        std::string htype = doc["HType"].GetString();

        if (htype != "flat")
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `HType` should be equal 'flat'");
        ...
        if (cardinal != "END")
            throw std::runtime_error("FlatHorizon::fromJSON() - invalid JSON, `Cardinal` should be equal 'END'");

        std::string name = doc["Name"].GetString();

        std::array<float, 2> anchor{doc["Anchor"][0].GetFloat(), doc["Anchor"][1].GetFloat()};

        return FlatHorizon(depth, dip, azimuth, anchor, name);
    }
}
