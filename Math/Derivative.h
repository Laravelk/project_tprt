...
public:
  ...
  derivative_x(const float x, const float y,
               const _2D::BicubicInterpolator<double> &interpolator) {
    return (interpolator(x + EPS, y) - interpolator(x - EPS, y)) * REVERSE_EPS_MUL_TWO;
  }

    static float derivative_xx(const float x, const float y, const _2D::BicubicInterpolator<double> &interpolator) {
      float fl = derivative_x(x - EPS, y, interpolator);
      float fr = derivative_x(x + EPS, y, interpolator);
      return (fr - fl) * REVERSE_EPS_MUL_TWO;
    }

    static float derivative_yy(const float x, const float y, const _2D::BicubicInterpolator<double> &interpolator) {
        float fl = derivative_y(x, y - EPS, interpolator);
        float fr = derivative_y(x, y + EPS, interpolator);
        return (fr - fl) * REVERSE_EPS_MUL_TWO;
  }

  static float derivative_xy(const float x, const float y, const _2D::BicubicInterpolator<double> &interpolator) {
      float fl = derivative_x(x, y - EPS, interpolator);
      float fr = derivative_x(x, y + EPS, interpolator);
      return (fr - fl) * REVERSE_EPS_MUL_TWO;
  }

  static float derivative_yx(const float x, const float y, const _2D::BicubicInterpolator<double> &interpolator) {
  ...
  static Eigen::Matrix2f calculate_hessian(const float x, const float y, const _2D::BicubicInterpolator<double> &interpolator) {
