...
public:
    ...
    explicit Optimize(ray_tracing::RayData *ray_data): ray_data(ray_data) {}
    ...
    double operator()(const VectorXd& x, VectorXd& grad)
    {
        ...
        ray_data->setTrajectory(xx);

        unsigned long number_of_unknown = trajectory.size();

        for (unsigned long i = 0; i < number_of_unknown - 2; i++) {
            float xk_1 = trajectory[i][0]; // xk-1
            float yk_1 = trajectory[i][1];
            float zk_1 = trajectory[i][2];
            ...
            std::array<float, 2> derive =
                    horizons[ray_code[i + 1].layerNumber]->getGradientInPoint(xk, yk);
...
//            std::cerr << "DzkDyk: " << DzkDyk << std::endl;
            ...
            float vk = vp[ray_code[i + 1].layerNumber];
...
//                    std::cerr << "vk: " << vk << std::endl;
            ...
            if (expression < 0) {
                ...
                vk_1 = vp[ray_code[i].layerNumber]; // vk_1
            }
...
//        }
        ...
        double calculate_full_time(ray_tracing::RayData *ray_data) {
            double time = 0;
...
//            std::cerr << trajectory[number_of_unknowns - 1][0] << " " <<trajectory[number_of_unknowns - 1][1] << " " << trajectory[number_of_unknowns - 1][2] << std::endl;
