...
namespace ray_tracing {
    ...
    Receiver Receiver::fromJSON(const rapidjson::Value &doc) {
        if (!doc.IsObject())
            throw std::runtime_error("Receiver::fromJSON() - document should be an object");

        std::vector<std::string> required_fields = {"Location", "Nc", "Orientation", "Cardinal", "Sampling", "Name"};

        for (const auto &field: required_fields) {
            if (!doc.HasMember(field.c_str()))
                throw std::runtime_error("Receiver::fromJSON() - invalid JSON, missing field " + field);
        }

        if (!doc["Location"].IsArray())
            throw std::runtime_error("Receiver::fromJSON() - invalid JSON, `Location` should be an array");

        if (doc["Location"].Size() != 3)
            throw std::runtime_error(
                    "Receiver::fromJSON - invalid JSON, wrong 'Location' size (should be equal three)");

        if (!doc["Nc"].IsUint64())
            throw std::runtime_error("Receiver::fromJSON - invalid JSON, `Nc` should be an unsigned integer");
        uint64_t nc = doc["Nc"].GetUint64();

        if (!doc["Orientation"].IsArray())
            throw std::runtime_error("Receiver::fromJSON() - invalid JSON, `Orientation` should be an array");

        if (doc["Orientation"].Size() != nc)
            throw std::runtime_error(
                    "Receiver::fromJSON - invalid JSON, wrong 'Orientation' size (should be equal Nc)");

        if (!doc["Cardinal"].IsString())
            throw std::runtime_error("Receiver::fromJSON() - invalid JSON, `Cardinal` should be a string");

        if (!doc["Sampling"].IsFloat())
            throw std::runtime_error("Receiver::fromJSON() - invalid JSON, `Sampling` should be a float");
        ...
        if (cardinal != "END")
            throw std::runtime_error("Receiver::fromJSON() - invalid JSON, `Cardinal` should be equal 'END'");
        ...
        for (uint64_t i = 0; i < nc; i++) {
            if (!doc["Orientation"][i].IsArray())
                throw std::runtime_error(
                        "Receiver::fromJSON() - invalid JSON, `Orientation` component should be an array");
            orientation.push_back({doc["Orientation"][i][0].GetFloat(), doc["Orientation"][i][1].GetFloat(),
