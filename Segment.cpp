...
Segment::Segment(const std::array<float, 3> &source_location,
                 ...
                 const Layer &layer, std::shared_ptr<Horizon> &ihorizon)
    : source_location(source_location), receiver_location(receiver_location),
      layer(layer), horizon(std::move(ihorizon.get())) {}
...
const std::unique_ptr<Horizon> &Segment::getHorizon() const { return horizon; }
