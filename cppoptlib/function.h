...
template <class scalar_t, class vector_t, class matrix_t> struct State {
  int dim;
  int order;
  ...
  State(const int dim, const int order)
      : dim(dim), order(order), x(vector_t::Zero(dim)),
        gradient(vector_t::Zero(dim)), hessian(matrix_t::Zero(dim, dim)) {}
...
template <class TScalar, int TDim = Eigen::Dynamic> class Function {
public:
