...
public:
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx)
    {
        ...
        fx = f(x, m_grad);
//        std::cerr << std::endl << m_grad << std::endl << std::endl;
        Scalar gnorm = m_grad.norm();
//        std::cerr << "optimize" << std::endl;
//        std::cerr << gnorm << " " << m_param.epsilon << " " << m_param.epsilon_rel * x.norm() << " " << 1 << std::endl;
        if(fpast > 0)
            m_fx[0] = fx;
        ...
        for( ; ; )
        {
            ...
            gnorm = m_grad.norm();
//            std::cerr << "optimize" << std::endl;
//            std::cerr << gnorm << " " << m_param.epsilon << " " << m_param.epsilon_rel * x.norm() << " " << k << std::endl;
